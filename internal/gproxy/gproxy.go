...
const P2PROXY_Protocol = "/p2p/proxy/1.0.0"
...
func P2ProxyService(h host.Host, proxyAddr ma.Multiaddr, dest peer.ID) *P2ProxyServer {
	...
	h.SetStreamHandler(P2PROXY_Protocol, P2ProxyStreamHandler)
	fmt.Println("p2p proxy server is ready...")
	fmt.Println("p2p proxy address is :")
	for _, a := range h.Addrs() {
		fmt.Printf("%s/ipfs/%s\n", a, peer.Encode(h.ID()))
	}
	return &P2ProxyServer{
		host:      h,
		dest:      dest,
		proxyAddr: proxyAddr,
	}
...
func P2ProxyStreamHandler(stream network.Stream) {
	...
	hq := strings.Split(request.Host, ":")
	if len(hq) > 1 && hq[1] == "443" {
	...
	forwardRequest := new(http.Request)
	*forwardRequest = *request
	fmt.Printf("begin to make forward request to %s", request.URL)
	response, err := http.DefaultTransport.RoundTrip(forwardRequest)
	if err != nil {
		stream.Reset()
		log.Errorln(err)
		return
...
func (p *P2ProxyServer) ServeHTTP(rep http.ResponseWriter, req *http.Request) {
	fmt.Printf("proxying request for %s to peer %s\n", req.URL, p.dest.Pretty())
	stream, err := p.host.NewStream(context.Background(), p.dest, P2PROXY_Protocol)
	if err != nil {
		log.Errorln(err)
	...
	rep.WriteHeader(resp.StatusCode)
	io.Copy(rep, resp.Body)
	resp.Body.Close()
