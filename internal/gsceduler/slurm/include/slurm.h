 ...
 */
#define SLURM_VERSION_NUMBER 0x140200
 ...
 * "bgl[123x456]". This selects all nodes between 1 and 4 inclusivef
...
typedef struct job_info {
	...
	time_t derived_ec;	/* time job is eligible for running */
...
typedef struct {
	...
	uint32_t pack_step_cnt;	/* total step count for entire pack job */
...
/*
 * NOTE: slurmdb_reservation_rec_t used to communicate with slurmdbd uses
 * an uint32_t to store the flag (vs. uint64_t in slurmctld_resv_t used in
 * slurmctld), so any additional flags in 19.05 will not be captured by the
 * database.
 */
